...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub CreateCalender()
		  // Setup Calendar Parameters for PopUp Demo, Caution: This Array is destroyed by Calendar Close!
		  ...
		  para.AutoCollapse           = True                                        // If False, the calendar must be closed using the Close method ESC or Close 
		  para.EnableWheel            = True
		  
		  
		  CalendarContainer = WTEC_Calendar.CreateCalendar(Self.PB_Popup, para)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub PopUpDemo_ReportsClosed(d as datetime, IsChanged as boolean)
	...
	#tag Method, Flags = &h21
		Private Sub PopUpDemo_ReportsNewDate(year as integer, month as integer, day as integer)
	...
	#tag Method, Flags = &h21
		Private Sub PopUpDemo_ReportsNewTime(hour as integer, minute as integer, Seconds as integer)
...
#tag Events PB_Popup
	#tag Event
		Sub Pressed()
		  ...
		  If CalendarContainer <> Nil Then Return
